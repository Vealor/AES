...
//   -fix subbytes and isubbytes (indexing is wrong i think)
...
class Utils {
    ...
    private static void keyExpansionCore(byte[] prevKey, int i) {
        ...
        // apply s-box
        for(int j = 0; j < 4; j++) {
            prevKey[j] = Utils.sBox[prevKey[j] + 128];
        }

        // xor first byte with rcon
        prevKey[0] ^= Utils.rcon[i];
    }
    ...
    protected static void expandKey(byte[] fullKey) {
        ...
        for(i = 1; i < 7; i++) {
            ...
            for(int j = 0; j < 4; j++) {
                fullKey[ndx + j] = Utils.sBox[fullKey[ndx + j] + 128];
    ...
    protected static void subBytes(byte[] input) {
        // input length is checked to be 16
        for(int i = 0; i < input.length; i++) {
            input[i] = Utils.sBox[input[i] + 128];
        }
    }
    ...
    protected static void iSubBytes(byte[] input) {
        // input length is checked to be 16
        for(int i = 0; i < input.length; i++) {
            input[i] = Utils.iSBox[input[i] + 128];
        }
    }
    ...
    protected static void iShiftRows(byte[] input) {
        ...
        Utils.oneDtwoD(input, shifted);
        
        for(int i = 0; i < 4; i++) {
            Utils.rotateLeft(shifted[i], 4 - i);
        }
        
        Utils.twoDoneD(shifted, input);
    }
     ...
     *  ‾  ‾     ‾           ‾  ‾  ‾ 
     */
    protected static void mixColumns(byte[] input) {
    ...
    protected static void iMixColumns(byte[] input) {
        System.out.println("iMixColumns unimplemented");
    }
}
...
// reference for order of operations: http://www.cs.columbia.edu/~sedwards/classes/2008/4840/reports/AES.pdf
class Decrypter {
    private byte[] key;
    private BufferedReader inFile;
    private FileOutputStream outFile;
